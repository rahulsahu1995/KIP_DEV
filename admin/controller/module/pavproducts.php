...
class ControllerModulepavproducts extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			unset(  $this->request->post['action_mode']	 );
 			...
 			$this->model_setting_setting->editSetting('pavproducts', $this->request->post);		
		...
		if (isset($this->error['dimension'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pavproducts', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pavproducts', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['pavproducts_module'])) {
			$this->data['modules'] = $this->request->post['pavproducts_module'];
		} elseif ($this->config->get('pavproducts_module')) { 
			$this->data['modules'] = $this->config->get('pavproducts_module');
		}	
		...
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavproducts.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavproducts')) {
