...
class ControllerModulePavblog extends Controller {
	...
	public function getModules( $list_files = array() ){
		...
		$this->load->model('pavblog/blog');
		$extensions = $this->model_pavblog_blog->getExtensions('module');	
		$module_data = array();
		$this->load->model('pavblog/menu');
		//$t = $this->model_pavblog_menu->getDropdown(null,0, 'pavblogcategory_module[][category_id]' );
		foreach ($extensions as $extension) {
			if(isset($list_files[$extension['code']]) && $this->user->hasPermission('modify', 'module/'.$extension['code'])){
				$tmp_modules = array();
					...
					foreach ($modules as $index => $module) {
						switch ($extension['code']) {
							case 'pavblogcategory':
								...
								$module['menus'] = $this->model_pavblog_menu->getDropdown(null, $module['category_id'], $extension['code'].'_module['.($index).'][category_id]' );
								break;
				...
				$module_data[$extension['code']] = $tmp_modules;
			}elseif(isset($list_files[$extension['code']]) && !$this->user->hasPermission('modify', 'module/'.$extension['code']))
				unset($list_files[$extension['code']]);
