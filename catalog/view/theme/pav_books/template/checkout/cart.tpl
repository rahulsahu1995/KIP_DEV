...
<section class="span<?php echo $SPAN[1];?>">
	<?php if ($attention) { ?>
	<div class="attention"><?php echo $attention; ?><img src="catalog/view/theme/<?php echo $themeName;?>/image/close.png" alt="" class="close" /></div>
	<?php } ?>
	<?php if ($success) { ?>
	<div class="success"><?php echo $success; ?><img src="catalog/view/theme/<?php echo $themeName;?>/image/close.png" alt="" class="close" /></div>
	<?php } ?>
	<?php if ($error_warning) { ?>
	<div class="warning"><?php echo $error_warning; ?><img src="catalog/view/theme/<?php echo $themeName;?>/image/close.png" alt="" class="close" /></div>
	<?php } ?>

	<div id="content" class="cart one-page">	
					...
					<table class="table no-border">
						...
						<tbody>
								...
								<td class="name">
									...
									<div class="avg-right">
										...
										<?php if ($product['reward']) { ?>
											<p><small><?php echo $product['reward']; ?></small></p>
										<?php } ?>
									</div>
								</td>
									...
									<div class="row-fluid">
										<div class="span7">
											<div class="cart-inner">
												<input type="text" name="quantity[<?php echo $product['key']; ?>]" value="<?php echo $product['quantity']; ?>" size="1" class="span12" />		
											</div>											
										</div>									
										<div class="span5">
											<div class="cart-inner">
												<input class="pull-left cart-action" type="image" src="catalog/view/theme/<?php echo $themeName;?>/image/update.png" alt="<?php echo $button_update; ?>" title="<?php echo $button_update; ?>" />									
												<a class="pull-left cart-action" href="<?php echo $product['remove']; ?>">
							...
							<tr>
								...
								<td class="model"></td>
								<td class="quantity">
									<input type="text" name="" value="1" size="1" disabled="disabled" class="span12" />
									&nbsp;
			...
			<?php if ($coupon_status || $voucher_status || $reward_status || $shipping_status) { ?>
				<div  class="content">
					...
					<div class="wrapper">
						...
						<table class="radio table is-relative">
				...
				<div class="cart-module clearfix">
					<div id="coupon" class="content" style="display: <?php echo ($next == 'coupon' ? 'block' : 'none'); ?>;">
						<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="no-border">
							<p>
								<span><?php echo $entry_coupon; ?></span>
								<input type="text" name="coupon" value="<?php echo $coupon; ?>" class="span3" />
					...
					<div id="voucher" class="content" style="display: <?php echo ($next == 'voucher' ? 'block' : 'none'); ?>;">
						<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="no-border">
							<p>
								<span><?php echo $entry_voucher; ?></span>
								<input type="text" name="voucher" value="<?php echo $voucher; ?>" class="span3" />
					...
					<div id="reward" class="content" style="display: <?php echo ($next == 'reward' ? 'block' : 'none'); ?>;">
						<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data">
							...
							<p><input type="text" name="reward" value="<?php echo $reward; ?>" class="span6" /></p>
							<input type="hidden" name="next" value="reward" />
							<p><input type="submit" value="<?php echo $button_reward; ?>" class="button" /></p>
						</form>
					</div>
					<div id="shipping" class="content" style="display: <?php echo ($next == 'shipping' ? 'block' : 'none'); ?>;">
						...
						<div class="wrapper clearfix">
							<input type="button" value="<?php echo $button_quote; ?>" id="button-quote" class="button right" />	
						</div>							
					</div>
				</div>
				<?php } ?>
				<div class="cart-total wrapper clearfix">
					<table id="total" class="table no-border">
						<?php foreach ($totals as $total) { ?>
						<tr>
							<td class="a-left"><b><?php echo $total['title']; ?>:</b></td>
		...
		$('#button-quote').live('click', function() {
			$.ajax({
				...
				beforeSend: function() {
					...
					$('#button-quote').after('<span class="wait">&nbsp;<img src="catalog/view/theme/<?php echo $themeName;?>/image/loading.gif" alt="" /></span>');
				},
				complete: function() {
				...
				success: function(json) {
					...
					if (json['error']) {
						if (json['error']['warning']) {
							$('#notification').html('<div class="warning" style="display: none;">' + json['error']['warning'] + '<img src="catalog/view/theme/<?php echo $themeName;?>/image/close.png" alt="" class="close" /></div>');
